...
class TestHooks:
    ...
    def test_basic(self, cmd, mocker, commit):
        ...
        commit('Some message')
        result, repo_dir = cmd('hooks post-commit')
        assert result.exit_code == 0
    ...
    def test_task_static(self, cmd, mocker, commit):
        ...
        commit('Some message')
        result, repo_dir = cmd('hooks post-commit', config='task_static.config')
        assert result.exit_code == 0
    ...
    def test_task_dynamic_branch(self, cmd, mocker, commit):
        ...
        result, repo_dir = cmd('hooks post-commit', config='task_dynamic_branch.config')
        assert result.exit_code == 0
    ...
    def test_task_dynamic_commit(self, cmd, mocker, commit):
        ...
        result, repo_dir = cmd('hooks post-commit', config='task_dynamic_commit.config')
        assert result.exit_code == 0
    ...
    def test_project(self, cmd, mocker, commit):
        ...
        commit('Some message')
        result, repo_dir = cmd('hooks post-commit', config='project.config')
        assert result.exit_code == 0
